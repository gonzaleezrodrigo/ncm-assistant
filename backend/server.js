  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/health") {
      sendJson(res, 200, {
        ...
        webSearchEnabled: isOpenRouterWebSearchEnabled()
      });
      return;
    }
...
function buildSearchIndex(items) {
  return items.map(item => {
    ...
    const allTokens = tokenize(`${item.descripcionOficial} ${keywordText} ${questionText} ${item.capitulo || ""}`);
    return {
      item,
      normalizedDescription: normalize(item.descripcionOficial),
      normalizedKeywords: (item.keywords || []).map(normalize),
      tokenSet: new Set(allTokens.flatMap(tokenVariants))
    };
  });
}
...
function searchLocal(normalizedText) {
  ...
  const queryVariants = queryTokens.flatMap(tokenVariants);
  ...
  const candidates = searchIndex
    .map(entry => {
      ...
      for (const keyword of entry.normalizedKeywords) {
        ...
        const keywordVariants = keywordTokens.flatMap(tokenVariants);
        const phraseWeight = keywordTokens.length > 1 ? 22 : 16;

        if (normalizedText.includes(keyword)) {
          score += phraseWeight;
          reasons.push(keyword);
        } else if (keywordVariants.every(token => queryVariantSet.has(token))) {
          score += Math.max(8, phraseWeight - 5);
          reasons.push(keyword);
        }
      }

      for (const token of queryVariantSet) {
            ...
            plugins: [
              {
                ...
                max_results: 3,
                search_prompt:
                  "Busca fuentes relevantes sobre nomenclatura arancelaria, NCM, Sistema Armonizado y descripcion tecnica del producto. Usa los resultados solo como apoyo orientativo."
              }
            ]
          }
        : {}),
...
function isOpenRouterWebSearchEnabled() {
  return String(process.env.OPENROUTER_WEB_SEARCH || "").toLowerCase() === "true";
}
